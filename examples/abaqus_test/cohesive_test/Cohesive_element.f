...
C
C	Srinath Chakravarthy ---- Brown University 2011
C
C=========================== SUBROUTINE UEL ===================
C
...
C
C	This represents the parent nodes on which the cohesive element is defined
C	It can be viewed as a 1D element between the upper and lower surfaces of the element
C	The stiffness matrix is then obtained for this element (4 x 4)
C		This matrix is then broadcast to its parent elements dof's to an (8 x 8) matrix with 
C		the correct coefficients zeroed out
C				4----3
C				|	 |
C				1----2
C
...
C
         PRINT *, "Step = ", KSTEP, KINC
         PRINT *, "Time Step = ", DTIME
         PRINT *, "Cohesive Forces  are" 

         CALL SEPLAW_T(PROPS,GAP,VGAP,TRACT,DTDG,DTIME)
...
C
	     ...
	     DO K = 1,2
             TMP=SIG*F(N)*(TRACT(1)*RNM(K)+TRACT(2)*TANGENT(K))*W(I)*DET
...
C
       ...
       END
C
C===================== SUBROUTINE SEPLAW TRACTIONS =====================
      SUBROUTINE SEPLAW_T(PROPS,GAP,VGAP,TRACT,DTDGAP,DTIME)
C
      IMPLICIT DOUBLE PRECISION (A-H,O-Z)
C
      DIMENSION PROPS(5)
